...
export function StepResults({
  ...
  const results = useMemo(() => {
    ...
    if (initialShapes !== null) {
      filtered = filtered.filter((b) => initialShapes.has(b.shape));
    }
    if (initialFlex !== null) {
      filtered = filtered.filter((b) => matchesFlex(b.flex, initialFlex));
    }
    return getRecommendations(filtered, input);
  }, [allBoards, input, allBrandsSelected, selectedBrands, initialShapes, initialFlex]);
