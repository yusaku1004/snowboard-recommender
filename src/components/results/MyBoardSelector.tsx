...
export function MyBoardSelector({ boards, selectedBoard, onSelect, triggerClassName = "" }: MyBoardSelectorProps) {
    ...
    <>
      <button type="button" onClick={() => setIsOpen(true)} className={triggerClassName}>
        ...
        <span className="max-w-full truncate">
          {selectedBoard ? `${selectedBoard.model}と比較中` : "自分の板と比較"}
        </span>
      </button>
      ...
      >
        ...
        <div className="mb-3">
          <input
            ...
            className="w-full px-3 py-2.5 rounded-xl bg-white/[0.05] border border-white/10 text-white text-sm placeholder-slate-500 outline-none focus:border-sky-500/50"
          />
        </div>
        ...
        <div className="max-h-64 overflow-y-auto space-y-1 mb-4">
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm cursor-pointer transition-all duration-200 ${
                  isSelected
                    ...
                    : "bg-white/[0.04] text-slate-300 border border-transparent hover:bg-white/10"
                }`}
              >
          ...
          {filteredModels.length === 0 && (
            <p className="text-slate-400 text-center py-4 text-sm">該当するボードが見つかりません</p>
          )}
        </div>
        ...
        <div className="flex gap-2">
          {selectedBoard && (
            <button type="button" onClick={handleClear}
              className="flex-1 py-3 rounded-xl bg-white/[0.06] hover:bg-white/10 text-slate-400 font-medium transition-all cursor-pointer text-sm border border-white/10"
            >
              解除
            </button>
          )}
